...
folder are mapped to the application's root namespace, 'UserActivityHistogram'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "UserActivityHistogram"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'UserActivityHistogram' in the same way
as `"UserActivityHistogram/sass/src"`.
